	...
	class Anuncios {
		...
		public function pegarInformacoes($id){
			...
			while($resultadoAnuncio = mysql_fetch_assoc($queryAnuncio))
				$informacoes[$resultadoAnuncio["id"]]["info"] = $resultadoAnuncio;

			$anuncioProdutos = $this->pegarAnunciosProdutos($id);

			foreach($anuncioProdutos as $anuncioId => $produtos)
				foreach($produtos as $produtoId)
					$informacoes[$anuncioId]["produtos"][] = $produtoId;

			return (count($informacoes) == 1 ? $informacoes[$id] : $informacoes);
		}

		public function pegarAnunciosProdutos($id){
			$Funcao = new Funcao();

			$anunciosProdutos = array();

			$queryAnunciosProdutos = mysql_query("SELECT * FROM `anuncios_produtos` ".$Funcao->carregarSearchSQL("anuncio", is_array($id) ? $id : array($id))." ORDER BY ordem ASC");
			while($resultadoAnunciosProdutos = mysql_fetch_assoc($queryAnunciosProdutos)){
				$anuncioId = $resultadoAnunciosProdutos["anuncio"];
				$produtoId = $resultadoAnunciosProdutos["produto"];
				$anunciosProdutos[$anuncioId][] = $produtoId;
			}

			// return (count($anunciosProdutos) == 1 ? $anunciosProdutos[$id] : $anunciosProdutos);
			return $anunciosProdutos;
		}
		...
		public function exibirAnuncio($id){
			...
			$exibirAnuncio = '
				...
				<br>
				<a href="#" class="incluir_carrinho">Adicionar no Carrinho de Compras - '.$anuncio["id"].'</a>
			';
